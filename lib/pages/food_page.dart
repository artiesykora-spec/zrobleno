...
class FoodPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    animation: store,
    builder: (context, _) {
      final today = store.foods
          .where((food) => store.isToday(food.date))
          .toList();
      final calories = today.fold<double>(
        0,
  ...
  Future<void> _addSavedProduct(BuildContext context) async {
              ...
              DropdownButtonFormField<Product>(
                ...
                onChanged: (value) {
                  ...
                  setDialogState(() {
                    selected = value;
                    grams.text =
                        value.packageGrams?.toStringAsFixed(0) ?? '100';
                  });
                },
              ),
              const SizedBox(height: 12),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                ...
                saved = store.addFoodFromProduct(selected, value);
    ...
    try {
      await AiService(store.aiSettings).syncFood(saved!);
    } on AiServiceException {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Їжу збережено локально. Sheets зараз недоступний.'),
        ),
      );
    }
  }
